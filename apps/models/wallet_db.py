...
def add_supplier_page():
    """
    عرض صفحة تعميد المورد مع إصلاح وتحديث بنية جداول قاعدة البيانات السحابية تلقائياً.
    """
    
    # 🚀 هندسة الإصلاح الذاتي والمزامنة البنيوية المباشرة مع PostgreSQL السحابية لمنع أخطاء الجداول الناقصة
    try:
        # 1. التأكد من وجود حقل wallet_code في جدول الموردين
        db.session.execute(db.text("ALTER TABLE suppliers ADD COLUMN IF NOT EXISTS wallet_code VARCHAR(50) UNIQUE;"))
        
        # 2. التأكد من وجود حقل status في جدول المحافظ لتجنب انهيار الـ INSERT السحابي
    ...
    if request.method == 'POST':
        try:
            ...
            # ⚡ حجز الهوية في الجلسة أولاً لتجاوز قيود تكامل المفاتيح الأجنبية السحابية
            db.session.flush()

            # تأسيس كائن المحفظة نقيًا لتجنب أخطاء حقول قاعدة البيانات غير المحدثة
            new_wallet = Wallet(
                wallet_code=generated_wallet_code,
